...
async def turn_on_all_lights_concurrently():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        print(f"--- Turning ON all {len(LIGHT_IDS)} lights concurrently ---")
        start = time.monotonic()
 
        # Wrap each request in a Task so they all start running immediately,
        # then gather() waits for all of them to finish.
        tasks = [
            asyncio.create_task(set_light(client, light_id, "ON"))
        ...
        print("(Should be ~2.0s - the slowest light's delay - not 4.5s)")
...
if __name__ == "__main__":
    asyncio.run(turn_on_all_lights_concurrently())
